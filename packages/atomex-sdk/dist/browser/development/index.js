...
var MixedPriceManager = class {
  constructor(currenciesProvider, providersMap) {
    ...
    __publicField(this, "cache", new InMemoryCache({ absoluteExpirationMs: 1e3 * 30 }));
  }
...
var _BinancePriceProvider = class {
  constructor() {
    ...
    __publicField(this, "_allSymbols");
...
var AtomexSwapPreviewManager = class {
  constructor(atomexContext) {
    ...
    __publicField(this, "swapPreviewFeesCache", new InMemoryCache({ absoluteExpirationMs: 20 * 1e3 }));
    __publicField(this, "userInvolvedSwapsCache", new InMemoryCache({ slidingExpirationMs: 30 * 1e3 }));
  }
    ...
    return {
      ...
      from: {
        ...
        actual: actualOrderPreview ? {
          ...
          amount: swapPreviewParameters.isFromAmount ? swapPreviewParameters.amount : new BigNumber7(0),
  ...
  async calculateMakerFees(fromCurrency, fromNativeCurrency, toNativeCurrency, toInitiateFees, fromRedeemFees) {
    const [toInitiateFeeInInFromCurrency, fromRedeemFeeInFromCurrency] = await Promise.all([
      fromCurrency !== toNativeCurrency ? this.convertFeesFromNativeCurrencyToCustom(toInitiateFees, toNativeCurrency, fromCurrency) : toInitiateFees,
      fromCurrency !== fromNativeCurrency ? this.convertFeesFromNativeCurrencyToCustom(fromRedeemFees, fromNativeCurrency, fromCurrency) : fromRedeemFees
    ]);
    return {
      name: "maker-fee",
...
var Atomex = class {
  constructor(options) {
    ...
    __publicField(this, "balanceManager");
    __publicField(this, "priceManager");
    __publicField(this, "swapManager");
    __publicField(this, "wallets");
    __publicField(this, "atomexContext");
    __publicField(this, "swapPreviewManager");
    __publicField(this, "_isStarted", false);
...
var WalletsManager = class {
  constructor(atomexNetwork) {
    this.atomexNetwork = atomexNetwork;
    __publicField(this, "wallets", /* @__PURE__ */ new Set());
  }
  addWallet(wallet) {
...
var getRedeemRewardInNativeCurrency = async (currencyOrId, redeemFee, priceManager, blockchainProvider) => {
  ...
  const rewardForRedeem = toFixedBigNumber(redeemFee.estimated.multipliedBy(1 + p), Math.min(currency.decimals, 9), BigNumber9.ROUND_FLOOR);
...
var getRedeemRewardInToken = async (currencyOrId, redeemFee, priceManager, blockchainProvider) => {
  ...
  const rewardForRedeem = toFixedBigNumber(inNativeToken.estimated.multipliedBy(nativeTokenPriceInCurrency), Math.min(currency.decimals, 9), BigNumber9.ROUND_FLOOR);
...
var AtomexBlockchainProvider = class {
  constructor() {
    ...
    __publicField(this, "networkOptionsMap", /* @__PURE__ */ new Map());
    __publicField(this, "blockchainToolkitProviders", /* @__PURE__ */ new Set());
  }
...
var defaultMaxPriorityFeePerGasInWei = new BigNumber10(25e8);
var getGasPriceInWei = async (toolkit) => {
  const gasPrice = await toolkit.eth.getGasPrice();
  return new BigNumber10(gasPrice);
};
var getMaxFeePerGas = async (toolkit, maxPriorityFeePerGasInWei = defaultMaxPriorityFeePerGasInWei) => {
  ...
  const latestBlockBaseFeePerGasInWei = new BigNumber10(latestBlock.baseFeePerGas);
...
var _Web3AtomexProtocolMultiChain = class {
  ...
  async getInitiateFees(params) {
    ...
    const gasLimit = new BigNumber11(hasRewardForRedeem ? gasLimitOptions.withReward : gasLimitOptions.withoutReward);
...
__publicField(Web3AtomexProtocolMultiChain, "defaultMaxNetworkFeeMultiplier", new BigNumber11(1.2));
...
var Web3BlockchainToolkitProvider = class {
  constructor(blockchain, rpcUrl) {
    this.blockchain = blockchain;
    this.rpcUrl = rpcUrl;
    __publicField(this, "toolkitId", "web3");
    __publicField(this, "toolkit");
  }
...
var Web3BalancesProvider = class {
  ...
  async getNativeTokenBalance(address, currency, toolkit) {
    ...
    return numberToTokensAmount(new BigNumber12(balance), currency.decimals);
  }
...
var EthereumWeb3AtomexProtocolMultiChain = class extends Web3AtomexProtocolMultiChain {
  constructor(atomexNetwork, atomexProtocolOptions, atomexBlockchainProvider, walletsManager, priceManager) {
    ...
    this.atomexProtocolOptions = atomexProtocolOptions;
    __publicField(this, "type", "multi-chain");
  }
  initiate(_params) {
...
var ERC20EthereumWeb3AtomexProtocolMultiChain = class extends Web3AtomexProtocolMultiChain {
  constructor(atomexNetwork, atomexProtocolOptions, atomexBlockchainProvider, walletsManager, priceManager) {
    super("ethereum", atomexNetwork, atomexProtocolOptions, atomexBlockchainProvider, walletsManager, priceManager);
    this.atomexProtocolOptions = atomexProtocolOptions;
    __publicField(this, "type", "multi-chain-approvable");
  }
  approve(_params) {
    throw new Error("Method not implemented.");
  }
...
var testnetNativeEthereumWeb3AtomexProtocolMultiChainOptions = __spreadProps(__spreadValues({}, mainnetEthereumWeb3AtomexProtocolMultiChainOptions.ETH), {
...
var isFA12TezosCurrency = (currency) => {
  return currency.blockchain === "tezos" && currency.type === "fa1.2";
};
var isFA2TezosCurrency = (currency) => {
...
// src/tezos/utils/fa12helper.ts
var fa12helper_exports = {};
__export(fa12helper_exports, {
  wrapContractCallsWithApprove: () => wrapContractCallsWithApprove
});
var wrapContractCallsWithApprove = (options) => {
  const batch = options.toolkit.wallet.batch().withContractCall(options.tokenContract.methods.approve(options.approvedAddress, options.approvedAmount));
  options.contractCalls.forEach((call) => batch.withContractCall(call));
  return batch;
};

// src/tezos/utils/fa2helper.ts
var fa2helper_exports = {};
__export(fa2helper_exports, {
  wrapContractCallsWithApprove: () => wrapContractCallsWithApprove2
});
var wrapContractCallsWithApprove2 = (options) => {
  const batch = options.toolkit.wallet.batch().withContractCall(options.tokenContract.methods.update_operators([{
    add_operator: {
  ...
  options.contractCalls.forEach((call) => batch.withContractCall(call));
  batch.withContractCall(options.tokenContract.methods.update_operators([{
    remove_operator: {
      owner: options.ownerAddress,
      operator: options.approvedAddress,
      token_id: options.tokenId
...
// src/tezos/utils/index.ts
var mutezInTez = new BigNumber13(1e6);
...
var TaquitoBlockchainWallet = class {
  constructor(atomexNetwork, walletOrSecretKey, rpcUrl) {
    ...
    this.rpcUrl = rpcUrl;
    __publicField(this, "internalWallet");
...
// src/tezos/atomexProtocol/multiChain/taquitoAtomexProtocolMultiChain.ts
...
var TaquitoAtomexProtocolMultiChain = class {
  ...
  getInitiateFees(_params) {
    const estimated = new BigNumber14(this.atomexProtocolOptions.initiateOperation.fee).div(mutezInTez);
    const result = { estimated, max: estimated };
    return Promise.resolve(result);
  }
  async redeem(params) {
    const wallet = await this.getWallet(params.senderAddress);
    const contract = await wallet.toolkit.wallet.at(this.swapContractAddress);
  ...
  getRedeemFees(_params) {
    const estimated = new BigNumber14(this.atomexProtocolOptions.redeemOperation.fee).div(mutezInTez);
    const result = { estimated, max: estimated };
    return Promise.resolve(result);
  }
  async refund(params) {
  ...
  getRefundFees(_params) {
    const estimated = new BigNumber14(this.atomexProtocolOptions.refundOperation.fee).div(mutezInTez);
    const result = { estimated, max: estimated };
    return Promise.resolve(result);
  }
  ...
  async getTransaction(type, operation) {
...
// src/tezos/atomexProtocol/multiChain/tezosTaquitoAtomexProtocolMultiChain.ts
var TezosTaquitoAtomexProtocolMultiChain = class extends TaquitoAtomexProtocolMultiChain {
  ...
  async initiate(params) {
    const wallet = await this.getWallet(params.senderAddress);
    const contract = await wallet.toolkit.wallet.at(this.swapContractAddress);
    const operation = await contract.methodsObject.initiate({
      settings: {
...
// src/tezos/atomexProtocol/multiChain/fa12TezosTaquitoAtomexProtocolMultiChain.ts
...
var FA12TezosTaquitoAtomexProtocolMultiChain = class extends TaquitoAtomexProtocolMultiChain {
  ...
  async initiate(params) {
    ...
    const multiplier = new BigNumber15(10).pow(currency.decimals);
    const totalAmount = params.amount.multipliedBy(multiplier);
    const wallet = await this.getWallet(params.senderAddress);
    const [contract, tokenContract] = await Promise.all([
      wallet.toolkit.wallet.at(this.swapContractAddress),
      wallet.toolkit.wallet.at(currency.contractAddress)
    ]);
    const operation = await fa12helper_exports.wrapContractCallsWithApprove({
...
// src/tezos/atomexProtocol/multiChain/fa2TezosTaquitoAtomexProtocolMultiChain.ts
...
var FA2TezosTaquitoAtomexProtocolMultiChain = class extends TaquitoAtomexProtocolMultiChain {
  ...
  async initiate(params) {
    ...
    const multiplier = new BigNumber16(10).pow(currency.decimals);
    const wallet = await this.getWallet(params.senderAddress);
    const [contract, tokenContract] = await Promise.all([
      wallet.toolkit.wallet.at(this.swapContractAddress),
      wallet.toolkit.wallet.at(currency.contractAddress)
    ]);
    const operation = await fa2helper_exports.wrapContractCallsWithApprove({
      toolkit: wallet.toolkit,
...
var mainnetTZBTCTezosTaquitoAtomexProtocolMultiChainOptions = __spreadProps(__spreadValues({}, mainnetFA12TezosTaquitoAtomexProtocolMultiChainOptionsBase), {
  currencyId: "TZBTC",
  swapContractAddress: "KT1Ap287P1NzsnToSJdA4aqSNjPomRaHBZSr",
  swapContractBlockId: "900350",
  redeemOperation: __spreadProps(__spreadValues({}, mainnetFA12TezosTaquitoAtomexProtocolMultiChainOptionsBase.redeemOperation), {
    gasLimit: 18e4
  })
});
var mainnetKUSDTezosTaquitoAtomexProtocolMultiChainOptions = __spreadProps(__spreadValues({}, mainnetFA12TezosTaquitoAtomexProtocolMultiChainOptionsBase), {
  currencyId: "KUSD",
...
var mainnetUSDtTezosTaquitoAtomexProtocolMultiChainOptions = __spreadProps(__spreadValues({}, mainnetFA2TezosTaquitoAtomexProtocolMultiChainOptionsBase), {
  currencyId: "USDT_XTZ",
...
var testnetUSDtTezosTaquitoAtomexProtocolMultiChainOptions = __spreadProps(__spreadValues({}, testnetFA2TezosTaquitoAtomexProtocolMultiChainOptionsBase), {
  currencyId: "USDT_XTZ",
...
var mapQuoteDtoToQuote = (quoteDto) => {
  ...
  const quote = {
    ask: new BigNumber17(quoteDto.ask),
    bid: new BigNumber17(quoteDto.bid),
...
var mapSymbolDtoToSymbol = (symbolDto, currenciesProvider, defaultDecimals = 9) => {
  ...
  return {
    ...
    baseCurrency,
    quoteCurrency,
    minimumQty: new BigNumber17(symbolDto.minimumQty),
    decimals: {
...
var mapOrderBookEntryDtoToOrderBookEntry = (entryDto) => {
  const entry = {
    side: entryDto.side,
    price: new BigNumber17(entryDto.price),
...
var mapOrderDtoToOrder = (orderDto, exchangeSymbolsProvider) => {
  ...
  return {
    ...
    leaveQty: new BigNumber17(orderDto.leaveQty),
...
var mapSwapDtoToSwap = (swapDto, exchangeSymbolsProvider) => {
  ...
  const swap = {
    ...
    to,
    trade: {
      qty: new BigNumber17(swapDto.qty),
      price: new BigNumber17(swapDto.price),
      side: swapDto.side,
    ...
    counterParty: {
      ...
      requisites: __spreadProps(__spreadValues({}, swapDto.counterParty.requisites), {
        rewardForRedeem: new BigNumber17(swapDto.counterParty.requisites.rewardForRedeem)
      }),
    ...
    user: {
      ...
      requisites: __spreadProps(__spreadValues({}, swapDto.user.requisites), {
        rewardForRedeem: new BigNumber17(swapDto.user.requisites.rewardForRedeem)
      }),
...
var mapTradeDtosToTrades = (tradeDtos) => {
  const trades = tradeDtos.map((tradeDto) => ({
    ...
    price: new BigNumber17(tradeDto.price),
    qty: new BigNumber17(tradeDto.qty)
  }));
  return trades;
};
...
var AtomexBuilder = class {
  constructor(options, atomexContext = new DefaultAtomexContext(options.atomexNetwork)) {
    ...
    __publicField(this, "customWalletsManagerFactory");
...
var EthereumHelpers = class extends Helpers {
  ...
  parseInitiateParameters(transaction) {
    ...
    return {
      ...
      rewardForRedeem: new BigNumber18(this._web3.utils.toBN(params["_payoff"]).toString()),
      netAmount: new BigNumber18(this._web3.utils.toBN(transaction.value).sub(this._web3.utils.toBN(params["_payoff"])).toString())
    };
  }
  async validateInitiateTransaction(_blockHeight, txId, secretHash, receivingAddress, amount, payoff, minRefundTimestamp, minConfirmations = 2) {
    var _a;
    amount = new BigNumber18(amount);
    payoff = new BigNumber18(payoff);
...
var TezosHelpers = class extends Helpers {
  ...
  parseInitiateParameters(content) {
    ...
    return {
      ...
      netAmount: new BigNumber19(content.amount).minus(initiateParams["settings"]["payoff"]),
      rewardForRedeem: new BigNumber19(initiateParams["settings"]["payoff"])
    };
  }
  ...
  async validateInitiateTransaction(blockHeight, txID, secretHash, receivingAddress, amount, payoff, minRefundTimestamp, minConfirmations = 2) {
    amount = new BigNumber19(amount);
    payoff = new BigNumber19(payoff);
    const netAmount = amount.minus(payoff);
  ...
  async estimateInitiateFees(source) {
    const dummyTx = {
      ...
      rewardForRedeem: new BigNumber19(0),
      netAmount: new BigNumber19(100)
    };
...
import BigNumber20 from "bignumber.js";
var FA12Helpers = class extends TezosHelpers {
  ...
  parseInitiateParameters(content) {
    ...
    return {
      ...
      netAmount: new BigNumber20(initiateParams["totalAmount"]).minus(initiateParams["payoffAmount"]),
      rewardForRedeem: new BigNumber20(initiateParams["payoffAmount"])
    };
  }
};
...
import BigNumber21 from "bignumber.js";
var FA2Helpers = class extends TezosHelpers {
  ...
  parseInitiateParameters(content) {
    ...
    return {
      ...
      receivingAddress: initiateParams["participant"],
      refundTimestamp: dt2ts(initiateParams["refundTime"]),
      netAmount: new BigNumber21(initiateParams["totalAmount"]).minus(initiateParams["payoffAmount"]),
      rewardForRedeem: new BigNumber21(initiateParams["payoffAmount"])
    };
  }
