...
var ExchangeManager = class {
  ...
  async findOrderBookEntry(exchangeSymbol, side, orderType, amount, isBaseCurrencyAmount) {
    ...
    for (const entry of orderBook.entries) {
      if (entry.side === side)
        continue;
      const convertedAmount = isBaseCurrencyAmount ? amount : toFixedBigNumber(amount.div(entry.price), exchangeSymbol.decimals.baseCurrency, import_bignumber3.BigNumber.ROUND_FLOOR);
...
var AtomexSwapPreviewManager = class {
  ...
  async getMaxOrderPreview(fromCurrencyBalance, availableLiquidity) {
    const maxAmount = import_bignumber7.default.min(fromCurrencyBalance, availableLiquidity.from.amount);
  ...
  getOrCreateUserInvolvedSwapsCurrencyInfo(userInvolvedSwapsInfo, currencyId) {
    if (!userInvolvedSwapsInfo[currencyId])
      userInvolvedSwapsInfo[currencyId] = {
        ...
        swaps: [],
        swapIds: [],
        lockedAmount: new import_bignumber7.default(0)
      };
