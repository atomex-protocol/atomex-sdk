...
__export(src_exports, {
  ...
  ERC20EthereumWeb3AtomexProtocolMultiChain: () => ERC20EthereumWeb3AtomexProtocolMultiChain,
  EthereumWeb3AtomexProtocolMultiChain: () => EthereumWeb3AtomexProtocolMultiChain,
  ExchangeManager: () => ExchangeManager,
  FA12TezosTaquitoAtomexProtocolMultiChain: () => FA12TezosTaquitoAtomexProtocolMultiChain,
  FA2TezosTaquitoAtomexProtocolMultiChain: () => FA2TezosTaquitoAtomexProtocolMultiChain,
  ...
  TezosTaquitoAtomexProtocolMultiChain: () => TezosTaquitoAtomexProtocolMultiChain,
...
var convertSymbolAndSideToFromAndToSymbolCurrencies = (symbol, side, currencyAmount, baseCurrencyPrice, isBaseCurrencyAmount = true) => {
  const preparedBaseCurrencyPrice = converters_exports.toFixedBigNumber(baseCurrencyPrice, symbol.decimals.price, import_bignumber2.default.ROUND_FLOOR);
  ...
  if (isBaseCurrencyAmount) {
    ...
    preparedQuoteCurrencyAmount = converters_exports.toFixedBigNumber(preparedBaseCurrencyPrice.multipliedBy(preparedBaseCurrencyAmount), symbol.decimals.quoteCurrency, isBuySide ? import_bignumber2.default.ROUND_CEIL : import_bignumber2.default.ROUND_FLOOR);
  } else {
    ...
    preparedBaseCurrencyAmount = converters_exports.toFixedBigNumber(preparedQuoteCurrencyAmount.div(preparedBaseCurrencyPrice), symbol.decimals.baseCurrency, isBuySide ? import_bignumber2.default.ROUND_FLOOR : import_bignumber2.default.ROUND_CEIL);
  }
  const preparedQuoteCurrencyPrice = converters_exports.toFixedBigNumber(new import_bignumber2.default(1).div(preparedBaseCurrencyPrice), symbol.decimals.price, import_bignumber2.default.ROUND_FLOOR);
  const baseCurrency = {
...
var TimeoutScheduler = class {
  ...
  counterExpirationWatcherId;
  actionWatchers = /* @__PURE__ */ new Set();
  _counter = 0;
  get counter() {
...
var ExchangeManager = class {
  ...
  async findOrderBookEntry(exchangeSymbol, side, orderType, amount, isBaseCurrencyAmount) {
    ...
    for (const entry of orderBook.entries) {
      if (entry.side === side)
        continue;
      const convertedAmount = isBaseCurrencyAmount ? amount : toFixedBigNumber(amount.div(entry.price), exchangeSymbol.decimals.quoteCurrency, import_bignumber3.BigNumber.ROUND_FLOOR);
...
var AtomexSwapPreviewManager = class {
    ...
    return {
      ...
      from: {
        ...
        actual: actualOrderPreview ? {
          ...
          amount: swapPreviewParameters.isFromAmount ? swapPreviewParameters.amount : new import_bignumber7.default(0),
  ...
  async getMaxOrderPreview(actualOrderPreview, availableLiquidity, authorizedFromAddress, balanceIncludingFees, fromCurrencyInfo, fromNativeCurrencyInfo, errors, _warnings) {
    ...
    const maxAmount = import_bignumber7.default.min(balanceIncludingFees.minus(userInvolvedSwapsInfo.fromTotalAmount), availableLiquidity.from.amount);
  ...
  async getUserInvolvedSwapsInfo(userAddress, fromCurrencyId) {
    ...
    const fromTotalAmount = swaps.reduce((total, swap) => total.plus(swap.from.amount), new import_bignumber7.default(0));
...
var FA12TezosTaquitoAtomexProtocolMultiChain = class extends TaquitoAtomexProtocolMultiChain {
  ...
  async initiate(params) {
    ...
    const multiplier = new import_bignumber15.default(10).pow(currency.decimals);
...
var FA2TezosTaquitoAtomexProtocolMultiChain = class extends TaquitoAtomexProtocolMultiChain {
  ...
  async initiate(params) {
    ...
    const multiplier = new import_bignumber16.default(10).pow(currency.decimals);
    const wallet = await this.getWallet(params.senderAddress);
    const [contract, tokenContract] = await Promise.all([
      wallet.toolkit.wallet.at(this.swapContractAddress),
      wallet.toolkit.wallet.at(currency.contractAddress)
...
var mapQuoteDtoToQuote = (quoteDto) => {
  ...
  const quote = {
    ask: new import_bignumber17.default(quoteDto.ask),
    bid: new import_bignumber17.default(quoteDto.bid),
...
var mapSymbolDtoToSymbol = (symbolDto, currenciesProvider, defaultDecimals = 9) => {
  ...
  return {
    ...
    baseCurrency,
    quoteCurrency,
    minimumQty: new import_bignumber17.default(symbolDto.minimumQty),
    decimals: {
...
var mapOrderBookEntryDtoToOrderBookEntry = (entryDto) => {
  const entry = {
    side: entryDto.side,
    price: new import_bignumber17.default(entryDto.price),
...
var mapOrderDtoToOrder = (orderDto, exchangeSymbolsProvider) => {
  ...
  return {
    ...
    leaveQty: new import_bignumber17.default(orderDto.leaveQty),
...
var mapSwapDtoToSwap = (swapDto, exchangeSymbolsProvider) => {
  ...
  const swap = {
    ...
    to,
    trade: {
      qty: new import_bignumber17.default(swapDto.qty),
      price: new import_bignumber17.default(swapDto.price),
      side: swapDto.side,
    ...
    counterParty: {
      ...
      requisites: {
        ...
        rewardForRedeem: new import_bignumber17.default(swapDto.counterParty.requisites.rewardForRedeem)
      },
    ...
    user: {
      ...
      requisites: {
        ...
        rewardForRedeem: new import_bignumber17.default(swapDto.user.requisites.rewardForRedeem)
      },
...
var mapTradeDtosToTrades = (tradeDtos) => {
  const trades = tradeDtos.map((tradeDto) => ({
    ...
    price: new import_bignumber17.default(tradeDto.price),
    qty: new import_bignumber17.default(tradeDto.qty)
  }));
  return trades;
};
...
var _ExchangeWebSocketClient = class {
  ...
  _isStarted = false;
  reconnectScheduler = new TimeoutScheduler([1e3, 5e3, 3e4, 6e4], 12e4);
  get isStarted() {
    return this._isStarted;
  }
  async start() {
...
var EthereumHelpers = class extends Helpers {
  ...
  parseInitiateParameters(transaction) {
    ...
    return {
      ...
      rewardForRedeem: new import_bignumber18.default(this._web3.utils.toBN(params["_payoff"]).toString()),
      netAmount: new import_bignumber18.default(this._web3.utils.toBN(transaction.value).sub(this._web3.utils.toBN(params["_payoff"])).toString())
    };
  }
  async validateInitiateTransaction(_blockHeight, txId, secretHash, receivingAddress, amount, payoff, minRefundTimestamp, minConfirmations = 2) {
    var _a;
    amount = new import_bignumber18.default(amount);
    payoff = new import_bignumber18.default(payoff);
...
var import_utils23 = require("@taquito/utils");
...
var TezosHelpers = class extends Helpers {
  ...
  parseInitiateParameters(content) {
    ...
    return {
      ...
      netAmount: new import_bignumber19.default(content.amount).minus(initiateParams["settings"]["payoff"]),
      rewardForRedeem: new import_bignumber19.default(initiateParams["settings"]["payoff"])
    };
  }
  ...
  async validateInitiateTransaction(blockHeight, txID, secretHash, receivingAddress, amount, payoff, minRefundTimestamp, minConfirmations = 2) {
    amount = new import_bignumber19.default(amount);
    payoff = new import_bignumber19.default(payoff);
    const netAmount = amount.minus(payoff);
  ...
  encodePublicKey(pubKey) {
    ...
    switch (curve) {
      case "ed":
        return Buffer.from((0, import_utils23.b58cdecode)(pubKey, import_utils23.prefix["edpk"])).toString("hex");
      case "p2":
        return Buffer.from((0, import_utils23.b58cdecode)(pubKey, import_utils23.prefix["p2pk"])).toString("hex");
      case "sp":
        return Buffer.from((0, import_utils23.b58cdecode)(pubKey, import_utils23.prefix["sppk"])).toString("hex");
      default:
  ...
  encodeSignature(signature) {
    ...
    if (Object.prototype.hasOwnProperty.call(import_utils23.prefix, pref)) {
      return Buffer.from((0, import_utils23.b58cdecode)(signature, (_a = Object.getOwnPropertyDescriptor(import_utils23.prefix, pref)) == null ? void 0 : _a.value)).toString("hex");
    }
  ...
  async estimateInitiateFees(source) {
    const dummyTx = {
      ...
      rewardForRedeem: new import_bignumber19.default(0),
      netAmount: new import_bignumber19.default(100)
    };
  ...
  isValidAddress(address) {
    return (0, import_utils23.validateAddress)(address) == import_utils23.ValidationResult.VALID;
  }
  getBlock(blockId) {
...
var FA12Helpers = class extends TezosHelpers {
  ...
  parseInitiateParameters(content) {
    ...
    return {
      ...
      netAmount: new import_bignumber20.default(initiateParams["totalAmount"]).minus(initiateParams["payoffAmount"]),
      rewardForRedeem: new import_bignumber20.default(initiateParams["payoffAmount"])
    };
  }
};
...
var FA2Helpers = class extends TezosHelpers {
  ...
  parseInitiateParameters(content) {
    ...
    return {
      ...
      receivingAddress: initiateParams["participant"],
      refundTimestamp: dt2ts(initiateParams["refundTime"]),
      netAmount: new import_bignumber21.default(initiateParams["totalAmount"]).minus(initiateParams["payoffAmount"]),
      rewardForRedeem: new import_bignumber21.default(initiateParams["payoffAmount"])
    };
  }
