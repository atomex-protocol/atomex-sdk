...
var AtomexSwapPreviewManager = class {
  ...
  async getSwapPreviewAccountData(_swapPreviewParameters, actualOrderPreview, availableLiquidity, fromCurrencyInfo, fromNativeCurrencyInfo, toCurrencyInfo, toNativeCurrencyInfo, swapPreviewFees, errors, warnings) {
    ...
    if (fromAddress) {
      ...
      const fromMinAvailableAmount = import_bignumber7.default.min(fromCurrencyInfo.currency.id === fromNativeCurrencyInfo.currency.id ? fromCurrencyBalance.minus(maxFromNativeCurrencyFee) : fromCurrencyBalance, availableLiquidity.from.amount);
